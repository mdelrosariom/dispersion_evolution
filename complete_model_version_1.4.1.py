...
def simple_landscape(nrow,ncol,size,shape):
   ...
   #CORRECTION, THE 12-4 THE ISLAND NEEDS TO BE DEFINED IN THE SEA (NOT THE TOTAL NUMBER 
   #OF COLUMNS, BUT THE SEA)
    dim = math.floor((ncol-main)*size)   #(ncol-main) all that is not mainland: i.e. island 
    ...
    return landscape
mainland_island = simple_landscape(ncol, nrow, size, 0)    #global
...
class Metapopulation:
    ...
    def initialize_pop(self):
        ...
        '''
        #mainland_island = simple_landscapex(15, 15, 1/2, 0) #modiffff 12_04
        startpop = 100
        for n in range(startpop):
            ...
            disp_cap_ini = rnd.randint(math.floor(min(nrow,ncol)/6), math.floor(min(nrow,ncol)/2) )  #change for gauss if necessary. this is lamnda that will be on the poisson distr
